...
forecast_steps = 365 * 24  # Forecast for the next year (hourly)
...
# Default feature
if st.button("Default Traffic Prediction"):
    st.subheader("Predicted Traffic Volume for the Next Year")
    traffic_forecast = forecast_df[['mean']].resample('D').mean()
...
if st.button("View Hourly Traffic Data"):
    ...
    # Option to download the filtered hourly data
    hourly_csv = forecast_df.to_csv(index=True)
    st.download_button("Download Hourly Traffic Data", data=hourly_csv, file_name="hourly_traffic_forecast.csv", mime="text/csv")
